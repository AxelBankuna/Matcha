...
$myquery = "SELECT A.id, A.username, A.dob_year, A.fame_rating, A.activity, D.*, ((2 * ASIN(POWER((POWER((SIN((RADIANS($lat_loc) - RADIANS(A.geo_lat))/2)), 2))
...
if ($sex_pref == "Bisexual") {
    $myquery .= " AND A.id != :id ORDER BY A.fame_rating DESC, coord LIMIT 3";
    $array = array(':id' => $id);
}
